...
//	public static String w_vehicle_license="浙A12345";
	...
	public static void  W_PassIllegal(int i) throws InterruptedException{
...
//		违规小类
		Mthead.OnClick(myYaml.getElement("w_violation_type"));
		String[] replace = { i + "" };
		Mthead.OnClick(myYaml.getValueByKey("w_violation_select_type", replace));
		
		Mthead.SendKey(myYaml.getElement("w_save_loss"), w_save_loss);
		//客货
...
//		Mthead.SendKey(myYaml.getElement(""), w_date);
		...
		Mthead.OnClick(myYaml.getElement("w_alter_btn"));
		 ...
		 try {
			   Assert.assertEquals(w_date, Mysql.Query2("n_date"));
//			   Assert.assertEquals(w_time, Mysql.Query1("n_time"));
			   Assert.assertEquals("岭下朱1511", Mysql.Query2("vc_station_name"));
			   Assert.assertEquals("何华", Mysql.Query2("vc_op_name"));
			   Assert.assertEquals(w_lane, Mysql.Query2("n_lane"));
			   Assert.assertEquals(w_serial, Mysql.Query2("n_serial"));
			   //班次 
			   Assert.assertEquals("1", Mysql.Query2("n_shift_code"));
			   Assert.assertEquals("1", Mysql.Query2("vc_vehicle_license_color"));
			   Assert.assertEquals(w_vehicle_license, Mysql.Query2("vc_vehicle_license"));
			   Assert.assertEquals("1", Mysql.Query2("vc_violation_type").split("0")[2]);
			   Assert.assertEquals(w_save_loss, Mysql.Query2("d_save_loss"));
			   Assert.assertEquals("0", Mysql.Query2("c_category"));
			   Assert.assertEquals("3", Mysql.Query2("c_vehicle_class"));
			   Assert.assertEquals(w_card, Mysql.Query2("vc_card_id"));
			   Assert.assertEquals(w_mark, Mysql.Query2("vc_marks"));
